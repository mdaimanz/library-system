...
You can refer to [Swagger UI](http://localhost:8080/swagger-ui/index.html) for an
interactive guide on how to use the API. After starting the application, Swagger
UI lists every endpoint, request schema, response, status code, and example. You
can also use its **Try it out** feature to send requests directly to the running
API.

The OpenAPI document generated by the running application is available as JSON
at [http://localhost:8080/v3/api-docs](http://localhost:8080/v3/api-docs). If the
server port is changed, replace `8080` in both URLs with the configured port.
